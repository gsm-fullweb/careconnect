...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "grammar") {
      ...
      userPrompt = `No seguinte texto, encontre espaços naturais (ou crie pequenas inserções lógicas de 1 ou 2 palavras relacionadas) e sugira links. Formate os links com tags HTML: <a href="URL_AQUI">âncora da vez</a>. Substitua URL_AQUI por "https://careconnect.com.br/plans" se recomendar checar como funciona ou "https://careconnect.com.br/contact" se for para falar com consultor. Adicione o texto com os links injetados e, no final, cite 1 referência de fonte confiável externa (organizações de saúde) caso faça sentido para o contexto. Retorne APENAS HTML puríssimo. Texto:\n\n${text}`;
...
Você deve SEMPRE mencionar o "Encontre um cuidador", o atendimento da CareConnect que ajuda as famílias a encontrarem cuidadores via WhatsApp.
...
- NÃO use <h1> (o título principal é gerado pelo sistema).`;
      ...
      userPrompt = `Escreva um artigo de blog completo, de alta autoridade e com mais de 1000 palavras sobre o tema: "${text}". O texto deve ser tão detalhado e rico quanto uma reportagem de capa do G1, focado em ajudar famílias que estão passando por este desafio agora. Lembre-se de mencionar o "Encontre um cuidador" (atendimento da CareConnect via WhatsApp) como solução facilitadora.`;
    }
