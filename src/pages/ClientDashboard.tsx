...
const ClienteDashboard = () => {
  ...
  const loadFilterOptions = async () => {
    try {
      const { data, error } = await supabase
        .from('vw_cuidadores_publicos')
        .select('cidade,cargo')
        .order('id', { ascending: false });

      if (error) throw error;

      const visibleCaregivers = data || [];
  ...
  const handleDemonstrarInteresse = async (cuidador: any) => {
    ...
    try {
      ...
      const novaObservacao = `[${dataHora}] Demonstrou interesse no cuidador ${cuidador.nomePublico || `ID ${cuidador.id}`}. Direcionado para a Mila.`;
      ...
      toast({
        ...
        description: `Vamos conectar você à nossa assistente Mila para prosseguir.`,
      });

      const whatsappHref = getWhatsAppHref(
        "551148633976",
        `Olá Mila, tenho interesse no cuidador(a) ${cuidador.nomePublico} (ID: ${cuidador.id}) que vi no painel.`
      );
                ...
                <Card>
                  ...
                  <CardContent>
                    {!buscaRealizada ? (
                      ...
                      <div className="text-center py-12 bg-purple-50 rounded-xl border border-purple-100 mt-4">
                        <User className="w-16 h-16 mx-auto mb-4 text-purple-300" />
                        <h3 className="text-lg font-bold text-gray-900 mb-2">Nenhum profissional disponível</h3>
                        <p className="text-gray-600 mb-6 max-w-md mx-auto">
                          Não encontramos cuidadores com esses critérios exatos. Mas nossa equipe pode ajudar a encontrar o perfil ideal.
                        </p>
                        <Button 
                          onClick={() => window.open(getWhatsAppHref('551148633976', 'Olá Mila, não encontrei cuidadores na busca e preciso de ajuda.'), '_blank')}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                        {cuidadoresEncontrados.map((cuidador) => (
                          <div key={cuidador.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                            <div className="p-5">
                              <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-3">
                                  <div className="w-12 h-12 bg-purple-100 rounded-full flex items-center justify-center text-purple-700 font-bold text-lg">
                                    {cuidador.nomePublico.charAt(0).toUpperCase()}
                                  </div>
                                  <div>
                                    <h3 className="font-bold text-gray-900 text-lg">{cuidador.nomePublico}</h3>
                                    <span className="inline-block bg-blue-50 text-blue-700 text-xs px-2 py-1 rounded font-medium mt-1">
                                      {cuidador.cargo}
                                    </span>
                                  </div>
                                </div>
                                ...
                                >
                                  <Heart className={`w-5 h-5 ${favoritos.includes(cuidador.id) ? 'fill-red-500 text-red-500' : ''}`} />
                                </Button>
                              </div>
                              ...
                              <div className="space-y-2 mb-4 text-sm text-gray-600">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span>{cuidador.cidade}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <Search className="w-4 h-4 text-gray-400" />
                                  <span>{cuidador.disponibilidade}</span>
                                </div>
                              </div>
