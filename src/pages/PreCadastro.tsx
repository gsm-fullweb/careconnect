...
const formSchema = z.object({
  ...
  termsAccepted: z.boolean().refine(val => val === true, { message: "Você precisa aceitar os termos de privacidade." }),
});
...
export default function PreCadastro() {
  ...
  const getFieldsForStep = (step: number) => {
    switch (step) {
      ...
      case 2: return ["cep", "city"];
      case 3: return ["education", "role", "experience_level", "has_experience"];
      case 4: return ["availability", "sleep_availability", "termsAccepted"];
      default: return [];
    }
  };
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const { data: caregiverRow, error: dbError } = await supabase.from("candidatos_cuidadores_rows").insert({
        ...
        endereco: normalizeCity(data.city),
        ...
        Declaracao: data.termsAccepted ? "Aceito" : "Pendente",
      ...
      if (caregiverRow?.id) {
        await syncCaregiverWithChathook({
          ...
          address: normalizeCity(data.city),
          role: data.role,
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                      ...
                      <FormField control={form.control} name="termsAccepted" render={({ field }) => (
                        <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4 shadow-sm bg-white">
                          <FormControl>
                            <input type="checkbox" checked={field.value} onChange={field.onChange} className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary mt-1" />
                          </FormControl>
